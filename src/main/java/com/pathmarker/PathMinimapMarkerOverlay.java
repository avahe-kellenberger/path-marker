...
public class PathMinimapMarkerOverlay extends Overlay {
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        Point minimapPoint = new Point(minimapWidgetLocation.getX() + minimapDrawWidget.getWidth() / 2,
                minimapWidgetLocation.getY() + minimapDrawWidget.getHeight() / 2);
        ...
        if ((config.hoverPathDisplaySetting() != PathMarkerConfig.PathDisplaySetting.NEVER)
                && (config.activePathDisplaySetting() == PathMarkerConfig.PathDisplaySetting.NEVER
                        || !plugin.isPathActive() || !config.drawOnlyIfNoActivePath())
                && (plugin.isKeyDisplayHoverPath()
                        || config.hoverPathDisplaySetting() == PathMarkerConfig.PathDisplaySetting.ALWAYS)) {
            for (WorldPoint worldPoint : plugin.getHoverPathTiles()) {
                if (config.hoverPathDrawLocations() == PathMarkerConfig.drawLocations.BOTH
                        || config.hoverPathDrawLocations() == PathMarkerConfig.drawLocations.MINIMAP) {
                    if (config.hoverPathDrawMode() == PathMarkerConfig.DrawMode.FULL_PATH
                            || worldPoint == plugin.getHoverPathTiles().get(plugin.getHoverPathTiles().size() - 1)) {
        ...
        if ((config.activePathDisplaySetting() != PathMarkerConfig.PathDisplaySetting.NEVER) && plugin.isPathActive()
                && (plugin.isKeyDisplayActivePath()
                        || config.activePathDisplaySetting() == PathMarkerConfig.PathDisplaySetting.ALWAYS)) {
            for (WorldPoint worldPoint : plugin.getActivePathTiles()) {
                if (config.activePathDrawLocations() == PathMarkerConfig.drawLocations.BOTH
                        || config.activePathDrawLocations() == PathMarkerConfig.drawLocations.MINIMAP) {
                    if (config.activePathDrawMode() == PathMarkerConfig.DrawMode.FULL_PATH
                            || worldPoint == plugin.getActivePathTiles().get(plugin.getActivePathTiles().size() - 1)) {
