...
public class Pathfinder {
    ...
    public Pair<List<WorldPoint>, Boolean> pathTo(int approxDestinationX, int approxDestinationY, int sizeX, int sizeY,
            int objConfig, int objID) {
        ...
        LocalPoint playerTrueTileLocalPoint = LocalPoint.fromWorld(client.getLocalPlayer().getWorldView(),
        ...
        while (index1 != index2) {
            ...
            // currentX is for the local coordinate while currentMapX is for the index in
            // the directions and distances arrays
            int currentMapX = currentX - middleX + offsetX;
            int currentMapY = currentY - middleY + offsetY;

            if (hasArrived(currentX, currentY, approxDestinationX, approxDestinationY, sizeX, sizeY, objConfig, objID,
                    collisionDataFlags)) {
                isReachable = true;
                break;
            }
        ...
        if (!isReachable) {
            // Try find a different reachable tile in the 21x21 area around the target tile,
            // as close as possible to the target tile
            ...
            for (int i = approxDestinationX - checkRange; i <= checkRange + approxDestinationX; ++i) {
                for (int j = approxDestinationY - checkRange; j <= checkRange + approxDestinationY; ++j) {
                    ...
                    if (currentMapX >= 0 && currentMapY >= 0 && currentMapX < 128 && currentMapY < 128
                            && distances[currentMapX][currentMapY] < 100) {
                        int deltaX = 0;
                        ...
                        if (distanceSquared < upperboundDistance || distanceSquared == upperboundDistance
                                && distances[currentMapX][currentMapY] < pathLength) {
        ...
        int directionNew;
        int directionOld;
        for (directionNew = directionOld = directions[currentX - middleX + offsetX][currentY - middleY
                + offsetY]; middleX != currentX
                        || middleY != currentY; directionNew = directions[currentX - middleX + offsetX][currentY
                                - middleY + offsetY]) {
    ...
    private boolean hasArrived(int baseX, int baseY, int targetX, int targetY, int sizeX, int sizeY, int objConfig,
            int objID, int[][] flags) {
        int objShape = -1;
        int objRot = 0;
        switch (objConfig) {
    ...
    private boolean reachRectangularBoundary(int[][] flags, int x, int y, int destX, int destY, int destWidth,
            int destHeight, int objectFlags) {
